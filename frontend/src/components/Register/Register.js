...
const RandomImage = lazy(() => import("../Login/RandomImage"));
...
const Register = () => {
  ...
  const registerHandler = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "/api/auth/register",
        { username, email, contact, address, password, type, dept },
        config
      );
  ...
  const onKeyUp = (e, target) => {
    //references for the input fileds
    if (e.keyCode === 13) {
      switch (target) {
        case "username":
          remail.focus();
